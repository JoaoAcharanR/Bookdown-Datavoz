...
Para más iformación sobre las opciones que se pueden agregar a los chunks visitar el [siguiente link](https://rmarkdown.rstudio.com/lesson-3.html)
...
Esta sección expone la manera y los elementos necesarios para crear un reporte automático del avance de los terrenos en las encuestas presenciales. Sin embargo, antes de generar el reporte debemos realizar modificaciones a la BBDD, las cuales se encuentran almacenadas en dos scripts. Dentro del reporte utilizamos la función `source()` para ejecutarlos automáticamente. En este sentido, **es imperante descargar los achivos que se presentan a continuación y almacenarlos en la carpeta de nuestro proyecto de R**.

**Scripts para construcción de reporte automático**

1) `Reporte_enc_presencial`: script automatizado
2) `df_viviendas`: BBDD con la selección final de dirección
3) `df_visitas`: BBDD con la cantidad de visitas que debieron realizar para lograr la encuesta en la dirección final
...
3. Cargar BBDD a R. Aquí utilizamos el segundo chunk y debemos ingresar el nombre de nuestro archivo.

4. Una vez tengamos listos los pasos anteriores, presionamos el botón ![](Imagenes/knit.jpg). Esto nos entergará el reporte en formato PDF dentro de la carpeta de nuestro proyecto de R.
